      ...
      <div class="page-header" id="banner">
        <div class="row">
          <div class="col-lg-8 col-md-7 col-sm-6">
           <img class="logo" src="<%=request.getContextPath() %>/resources/logo_image/상품 등록.png" style="width:450px; height:110px"><br>
          </div>
        </div>
      </div>
</div>
<center>
<form:form commandName="store" action="store_upload_proc.ad" method="post" enctype="multipart/form-data">
<table>
	<tr>
		<td colspan="3">
			<div class="form-group">
  			<label class="col-form-label mt-4" for="inputDefault">상품 카테고리</label>&nbsp;
  			<form:errors cssClass="err" path="s_code"/>
  			<select name="s_code" class="form-select">
				<option value="">선택</option>
				<option value="1" <c:if test="${store.s_code==1}">selected</c:if> >영화관람권</option>
				<option value="2" <c:if test="${store.s_code==2}">selected</c:if> >팝콘</option>
				<option value="3" <c:if test="${store.s_code==3}">selected</c:if> >음료</option>
				<option value="4" <c:if test="${store.s_code==4}">selected</c:if> >간식</option>
			</select>
			</div>
		</td>
	</tr>
	<tr>
		<td colspan="3">
			<div class="form-group">
      		...
      		<form:errors cssClass="err" path="s_image"/>
      		<input class="form-control" type="file" name="upload" id="formFile" value="${store.s_image}">
	...
	<tr>
		<td colspan="3">
			<div class="d-grid gap-2">
  			<button class="btn btn-lg btn-outline-primary" type="submit" id="sub">상품 등록</button>
			</div>
		</td>
	</tr>
