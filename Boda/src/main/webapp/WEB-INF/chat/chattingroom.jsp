...
<section class="chatWrap" id="chatLog">
		...
		<!-- chatting list by room_no -->
		  	<c:forEach var="clist" items="${chatList }">
		  		<!-- m1 == login.name -->
		  		<c:if test="${clist.m1_name eq loginInfo.name }">
			   		<!-- my msg -->
				...
				<c:if test="${clist.m1_name ne loginInfo.name }">
					<!-- another msg -->
					<div class="anotherMsg">
			  			<div class="d-inline-block align-middle" >
			   				<img src="<%= request.getContextPath() %>/resources/profile.png"
	   ...
	   <form id="chatForm">
			...
			<c:if test="${cb.send_mno ne loginInfo.no }">
			...
			<c:if test="${cb.send_mno eq loginInfo.no }">
